...
Critic=myCritic(compositLayers([Dim*2,Dim*3,Dim*3,Dim*3,Dim*2,Dim*2,Dim*2,Dim],0))
...
# %% Run with random
counter=0
train,test=Data.getPipeLine(p_miss=0.5,p_hints=0.5,train_rate=0.8,batch_ratio=0.5,repeat=1)
test=iter(test)
...
test=iter(test)
for dat_train,[mask,hint_mask,hints] in tqdm(train):
    Model2.train_critic(dat_train,mask,hints,steps=5)
    ...
    if counter%5:
        Model2.tensorboard_log('Step 2. Critic:',dat_train,mask,hints,hint_mask)
