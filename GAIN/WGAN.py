...
class WGAN(Orchestrator):
    ...
    def tensorboard_log(self,prefix,data_batch,mask,hints,hint_mask):
        ...
        self.critic.performance_log(self.summary_writer,prefix,data_batch,adjusted_generated_data,mask,hints,self.alpha,self.epoch)
        self.generator.performance_log_with_critic(self.summary_writer,prefix,data_batch,mask,hints,hint_mask,self.critic.criticise,self.epoch)
    ...
    def tensorboard_log_with_random(self,prefix,data_batch,mask,hints,hint_mask):
        ...
        genuine_critics=self.critic.criticise(data_batch,hints)
        ...
        with self.summary_writer.as_default():
            tf.summary.histogram(prefix+' generated_critics',generated_critics, step=self.epoch) 
            tf.summary.histogram(prefix+' genuine_critics',genuine_critics, step=self.epoch) 
    ...
    def train_critic_with_random(self,data_batch,mask,hints):
        ...
        generated_data=generate_random(data_batch,mask)
        adjusted_generated_data=mask*data_batch+generated_data*(1-mask)
        self.critic.train(data_batch,adjusted_generated_data,mask,hints,self.alpha,self.optimizer)
    ...
    def train_generator_with_critic(self,data_batch,mask,hints):
        with tf.GradientTape(persistent=True) as tape:
            generated_data=self.generator.generate(data_batch,mask)
            adjusted_generated_data=generated_data*(1-mask)+mask*data_batch
            critics=self.critic.criticise(adjusted_generated_data,hints)
